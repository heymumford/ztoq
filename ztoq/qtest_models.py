...
class QTestConfig(BaseModel):
    ...
    base_url: str = Field(
        default_factory=lambda: os.environ.get("qtest_base_url", ""),
    ...
    bearer_token: str = Field(
        default_factory=lambda: os.environ.get("qtest_bearer_token", ""),
...
class QTestAttachment(BaseModel):
    ...
    related_entity_type: str | None = Field(
        ...
        description="Type of entity this attachment is associated with",
    )
    ...
    def validate_content_type(cls, value):
        ...
        if value not in known_types and mime_category not in [
            "application",
            "text",
            "image",
            "audio",
...
class QTestPulseCondition(BaseModel):
    ...
    VALID_VALUE_TYPES: ClassVar[list[str]] = [
        "string",
        "number",
        "boolean",
        "array",
...
class QTestPulseConstant(BaseModel):
    ...
    name: str = Field(
        ...,
        description="Name of the constant (used as reference key)",
        min_length=1,
        max_length=255,
    )
...
class QTestScenarioFeature(BaseModel):
    ...
    def validate_content(cls, value):
        ...
        if not any(
            line.lstrip().startswith(("Scenario:", "Scenario Outline:"))
