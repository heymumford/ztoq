...
class SQLDatabaseManager:
    ...
    def save_folder(self, folder_model: FolderModel, project_key: str) -> None:
        ...
        with self.get_session() as session:
            ...
            folder_type = (
                folder_model.folderType
                if hasattr(folder_model, "folderType")
                else folder_model.folder_type
            )

            # Map parentId to parent_id (convert camelCase to snake_case)
            parent_id = (
                folder_model.parentId
                if hasattr(folder_model, "parentId")
                else folder_model.parent_id
            )
    ...
    def _save_custom_fields(
        ...
        for cf in custom_fields:
            ...
            existing_value = (
                ...
                .filter_by(field_id=field_def.id, entity_type=entity_type, entity_id=entity_id)
                .first()
            )
    ...
    def _save_attachments(
        ...
        for attachment in attachments:
            ...
            existing_attachment = (
                ...
                .filter_by(id=attachment_id, entity_type=entity_type, entity_id=entity_id)
                .first()
            )
    ...
    def _get_or_create_labels(self, session: Session, labels: list[str]) -> list[Label]:
        """
    ...
    def save_test_execution(self, test_execution_model: ExecutionModel, project_key: str) -> None:
        ...
        with self.get_session() as session:
            ...
            try:
                existing_execution = (
                    session.query(TestExecution).filter_by(id=test_execution.id).first()
                )
    ...
    def get_migration_state(self, project_key: str) -> MigrationState | None:
        """
    ...
    def create_entity_batch_state(
        ...
        with self.get_session() as session:
            ...
            if batch:
                ...
                batch.status = status
                batch.started_at = (
                    datetime.utcnow() if status == "in_progress" else batch.started_at
                )
    ...
    def get_incomplete_batches(
        ...
        with self.get_session() as session:
            ...
            return query.order_by(EntityBatchState.entity_type, EntityBatchState.batch_number).all()
    ...
    def get_statistics(self, project_key: str) -> dict[str, int]:
        ...
        with self.get_session() as session:
            ...
            # Environments
            stats["environments"] = (
                session.query(Environment).filter_by(project_key=project_key).count()
            )
            ...
            # Test cycles
            stats["test_cycles"] = (
                session.query(TestCycle).filter_by(project_key=project_key).count()
            )
            ...
            # Test executions
            stats["test_executions"] = (
                session.query(TestExecution).filter_by(project_key=project_key).count()
            )
            ...
            stats["test_steps"] = (
                ...
                .filter(TestCase.project_key == project_key)
                .count()
            )
            ...
            stats["custom_fields"] = (
                session.query(CustomFieldValue)
                .join(CustomFieldDefinition, CustomFieldValue.field_id == CustomFieldDefinition.id)
                .filter(CustomFieldDefinition.project_key == project_key)
                .count()
            )
