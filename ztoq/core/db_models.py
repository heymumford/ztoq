...
from sqlalchemy import (
    ...
    ForeignKey, Table, LargeBinary, Enum, Index, UniqueConstraint
)
...
class Status(Base):
    ...
    # Indexes
    __table_args__ = (
        Index('idx_status_project', 'project_key'),
...
class Environment(Base):
    ...
    # Indexes
    __table_args__ = (
        Index('idx_environment_project', 'project_key'),
    )
...
class CustomFieldDefinition(Base):
    ...
    # Indexes
    __table_args__ = (
        Index('idx_custom_field_project', 'project_key'),
    )
...
class CustomFieldValue(Base):
    ...
    id = Column(String(50), primary_key=True)
    field_id = Column(String(50), ForeignKey('custom_field_definitions.id', ondelete='CASCADE'), nullable=False)
...
class ScriptFile(Base):
    ...
    content = Column(Text)
    test_case_id = Column(String(50), ForeignKey('test_cases.id', ondelete='CASCADE'), nullable=False)
    ...
    # Indexes
    __table_args__ = (
        Index('idx_script_test_case', 'test_case_id'),
    )
...
class CaseVersion(Base):
    ...
    # Relationships - many-to-many with test cases
    test_cases = relationship("TestCase", secondary=case_version_association, back_populates="versions")
...
class TestStep(Base):
    ...
    status = Column(String(50))
    test_case_id = Column(String(50), ForeignKey('test_cases.id', ondelete='CASCADE'), nullable=True)
    test_execution_id = Column(String(50), ForeignKey('test_executions.id', ondelete='CASCADE'), nullable=True)
    
    # Relationships
    test_case = relationship("TestCase", back_populates="steps", foreign_keys=[test_case_id])
    test_execution = relationship("TestExecution", back_populates="steps", foreign_keys=[test_execution_id])
...
class TestCase(Base):
    ...
    versions = relationship("CaseVersion", secondary=case_version_association, back_populates="test_cases")
...
class TestCycle(Base):
    ...
    executions = relationship("TestExecution", back_populates="test_cycle", cascade="all, delete-orphan")
...
class TestExecution(Base):
    ...
    id = Column(String(50), primary_key=True)
    test_case_key = Column(String(50), ForeignKey('test_cases.key', ondelete='CASCADE'), nullable=False)
    ...
    def __repr__(self):
        return f"<TestExecution(id='{self.id}', case='{self.test_case_key}', status='{self.status}')>"
