...
class TestDatabaseStorageOperations:
    ...
    def test_complex_entity_creation(
        ...
        with db_manager.get_session() as session:
            ...
            custom_fields = (
                ...
                .filter_by(entity_type=EntityType.TEST_CASE, entity_id="TC-1")
                .all()
            )
    ...
    def test_parent_child_relationship_operations(self, db_manager, sample_project):
        ...
        with db_manager.get_session() as session:
            ...
            assert (
                len(children) == 3
            ), f"Expected 3 children by parent_id query, got {len(children)}"
            ...
            ), f"Expected 2 children after update, got {len(parent.children)}"
    ...
    def test_dataframe_operations(self, db_manager, sample_project):
        ...
        keys = {str(k) for k in df["key"].tolist()}
        assert "TEST" in keys
