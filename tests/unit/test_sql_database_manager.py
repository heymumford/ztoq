...
from ztoq.core.db_models import (
    Base, EntityType, Project, Folder, Status, Priority, Environment, Label,
    ...
    MigrationState, EntityBatchState
)
...
class TestSQLDatabaseManager:
    ...
    def test_create_engine(self, db_config):
        ...
        with patch('ztoq.core.db_manager.create_engine') as mock_create_engine:
            ...
            pg_manager = SQLDatabaseManager(config=pg_config)
    ...
    def test_initialize_database(self, db_config):
        ...
        essential_tables = [
            'projects', 'folders', 'test_cases', 'test_cycles', 'test_steps',
            'test_executions', 'custom_field_definitions', 'custom_field_values',
            'attachments', 'migration_state', 'entity_batch_state'
        ]
    ...
    def test_save_custom_fields(self, db_manager, test_project):
        ...
        with db_manager.get_session() as session:
            ...
            custom_fields = [
                ...
                CustomFieldModel(id="CF-5", name="Tags", type="multipleSelect", value=["api", "regression"])
            ]
    ...
    def test_save_links(self, db_manager):
        """Test saving links."""
        links = [
            LinkModel(id="LINK-1", name="Requirements", url="https://example.com/req", 
                     description="Requirements document", type="web"),
            LinkModel(id="LINK-2", name="Issue", url="https://example.com/issue/123", 
                     description="Related issue", type="issue")
        ]
        ...
        with db_manager.get_session() as session:
            ...
            saved_links = session.query(Link).filter_by(
                entity_type=EntityType.TEST_CASE,
                entity_id="TC-123"
            ).all()
            ...
            updated_links = session.query(Link).filter_by(
                entity_type=EntityType.TEST_CASE,
                entity_id="TC-123"
            ).all()
    ...
    def test_save_attachments(self, db_manager):
        ...
        with db_manager.get_session() as session:
            ...
            saved_attachments = session.query(Attachment).filter_by(
                entity_type=EntityType.TEST_EXECUTION,
                entity_id="EXEC-123"
            ).all()
    ...
    def test_get_or_create_labels(self, db_manager):
        ...
        with db_manager.get_session() as session:
            ...
            same_labels = db_manager._get_or_create_labels(
                session=session,
                labels=["Regression", "Smoke", "Performance"]
            )
    ...
    def test_save_test_case(self, db_manager, test_project, test_folder, test_priority):
        ...
        test_case = CaseModel(
            ...
            links=[
                LinkModel(name="Requirements", url="https://example.com/req", type="web")
            ],
            attachments=[],
            scripts=[]
        )
    ...
    def test_save_test_cycle(self, db_manager, test_project, test_folder):
        ...
        test_cycle = CycleInfoModel(
            ...
            links=[
                LinkModel(name="Sprint Board", url="https://example.com/sprint1", type="web")
            ],
            attachments=[]
        )
    ...
    def test_save_test_execution(self, db_manager, test_project):
        ...
        test_environment = EnvironmentModel(
            id="ENV-123",
            name="Production"
        )
        db_manager.save_environment(test_environment, test_project.key)
    ...
    def test_save_project_data(self, db_manager, test_project):
        ...
        folders = [
            ...
            FolderModel(id="FOLD-2", name="Test Cycles", folderType="TEST_CYCLE", projectKey="TEST")
        ]
        ...
        test_cycles = [
            CycleInfoModel(id="CYCLE-1", key="TEST-C1", name="Sprint 1 Testing", 
                         project_key="TEST", folder="FOLD-2")
        ]
        
        test_executions = [
            ExecutionModel(id="EXEC-1", testCaseKey="TEST-1", cycleId="CYCLE-1", 
                         status="PASS", environment="ENV-1")
        ]
        
        # Create fetch results
        fetch_results = {
            "project": FetchResult(entity_type="project", project_key="TEST", 
                                 items=[test_project], count=1, success=True),
            "folders": FetchResult(entity_type="folders", project_key="TEST", 
                                 items=folders, count=len(folders), success=True),
            "statuses": FetchResult(entity_type="statuses", project_key="TEST", 
                                  items=statuses, count=len(statuses), success=True),
            "priorities": FetchResult(entity_type="priorities", project_key="TEST", 
                                    items=priorities, count=len(priorities), success=True),
            "environments": FetchResult(entity_type="environments", project_key="TEST", 
                                      items=environments, count=len(environments), success=True),
            "test_cases": FetchResult(entity_type="test_cases", project_key="TEST", 
                                    items=test_cases, count=len(test_cases), success=True),
            "test_cycles": FetchResult(entity_type="test_cycles", project_key="TEST", 
                                     items=test_cycles, count=len(test_cycles), success=True),
            "test_executions": FetchResult(entity_type="test_executions", project_key="TEST", 
                                         items=test_executions, count=len(test_executions), success=True)
        }
    ...
    def test_save_project_data_with_placeholder(self, db_manager):
        ...
        test_cases = [
            CaseModel(id="TC-1", key="TEST-1", name="Test Login")
        ]
    ...
    def test_save_all_projects_data(self, db_manager):
        ...
        test_cases1 = [
            CaseModel(id="TC-1", key="PROJ1-1", name="Test 1 for Project 1")
        ]
        ...
        project1_results = {
            "project": FetchResult(entity_type="project", project_key="PROJ1", 
                                 items=[project1], count=1, success=True),
            "test_cases": FetchResult(entity_type="test_cases", project_key="PROJ1", 
                                    items=test_cases1, count=len(test_cases1), success=True)
        }
        
        project2_results = {
            "project": FetchResult(entity_type="project", project_key="PROJ2", 
                                 items=[project2], count=1, success=True),
            "test_cases": FetchResult(entity_type="test_cases", project_key="PROJ2", 
                                    items=test_cases2, count=len(test_cases2), success=True)
        }
        
        all_projects_data = {
            "PROJ1": project1_results,
            "PROJ2": project2_results
        }
    ...
    def test_get_statistics(self, db_manager, test_project):
        ...
        folder = FolderModel(id="FOLD-1", name="Test Cases", folderType="TEST_CASE", projectKey="TEST")
    ...
    def test_execute_query(self, db_manager, test_project):
        ...
        result = db_manager.execute_query("SELECT * FROM projects WHERE key = :key", {"key": "TEST"})
