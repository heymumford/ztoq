...
from ztoq.core.db_models import Base, Project, TestCase, Folder, Status, EntityType
...
class TestDatabaseErrorHandling:
    ...
    def test_connection_error_recovery(self, db_manager, test_project):
        ...
        try:
            ...
            with db_manager.get_session() as session:
                ...
                assert final_count == initial_count + 1, "Test case should be created after reconnection"
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if "database is locked" in str(e) and retry_count < max_retries - 1:
                        retry_count += 1
                        logger.info(f"Deadlock detected, retrying ({retry_count}/{max_retries-1})...")
    ...
    def test_orphaned_record_cleanup(self, db_manager, test_project):
        ...
        with db_manager.get_session() as session:
            ...
            for i in range(3):
                step = session.query(TestCase.steps.prop.mapper.class_).filter_by(
                    id=f"ORPHAN-STEP-{i}"
                ).first()
        ...
        try:
            for i in range(10):
                ...
                db_manager.update_migration_state(
                    test_project.key,
                    metadata={"processed_items": i + 1, "total_items": 10}
                )
