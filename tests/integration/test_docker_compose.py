...
class TestDockerComposeConfiguration:
    ...
    def docker_compose_file(self):
        ...
        compose_path = (
            Path(__file__).parent.parent.parent / "config" / "docker-compose.migration.yml"
        )
    ...
    def test_compose_version(self, docker_compose_file):
        ...
        assert docker_compose_file["version"] in [
            "3.8",
            "3.9",
            "3.7",
        ], "Version should be 3.7 or higher"
    ...
    def test_required_services(self, docker_compose_file):
        ...
        for service in required_services:
            assert (
                service in services
            ), f"Required service '{service}' not found in Docker Compose file"
    ...
    def test_migration_service_configuration(self, docker_compose_file):
        ...
        assert (
            len(db_vars) >= 5
        ), "migration should have database configuration environment variables"
    ...
    def test_migration_report_configuration(self, docker_compose_file):
        ...
        assert (
            report_service["build"]["dockerfile"] == "config/Dockerfile.migration-report"
        ), "Should use specialized Dockerfile"
