...
from ztoq.core.db_models import Base, Project, Folder, Status, TestCase, EntityBatchState
...
class TestDatabaseParallelOperations:
    ...
    def test_concurrent_mixed_operations(self, db_manager, test_project):
        ...
        assert all(isinstance(count, int) for count in results.get("read", [])), "Read operations should return counts"
        
        # Check that creates were successful
        assert results.get("create", []).count("created") > 0, "Some create operations should succeed"
        
        # Check that updates were successful or returned not_found (if the item was deleted)
        assert any(r == "updated" or r.startswith("not_found") for r in results.get("update", [])), \
               "Update operations should either succeed or not find the item"
        
        # Check that deletes were successful or returned not_found
        assert any(r == "deleted" or r.startswith("not_found") for r in results.get("delete", [])), \
    ...
    def test_parallel_transaction_isolation(self, db_manager, test_project):
        ...
        for case_id, old_status, new_status, success in results:
        ...
        with db_manager.get_session() as session:
            ...
            assert case.status in statuses, f"Final status should be one of {statuses}, got {case.status}"
