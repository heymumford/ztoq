...
class TestRunMigrationScript:
    ...
    def test_env_file_check(self, script_path):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert "Error: Environment file" in result.stdout, "Script should error on missing env file"
            assert "Please create an environment file" in result.stdout, "Script should provide guidance"
    ...
    def test_invalid_command(self, script_path):
        ...
        assert "Unknown command: invalid_command" in result.stdout, "Script should identify invalid command"
        assert "Run './run-migration.sh --help'" in result.stdout, "Script should suggest help command"
    ...
    def test_command_parsing(self, mock_run, script_path, temp_env_file):
        ...
        assert "--batch-size 100" in all_calls or "--batch-size=100" in all_calls, "Should use custom batch size"
        assert "--workers 10" in all_calls or "--max-workers 10" in all_calls, "Should use custom worker count"
        assert "--phase extract" in all_calls, "Should use specified phase"

    @patch('os.makedirs')
    @patch('os.chmod')
    @patch('subprocess.run')
    def test_setup_command_creates_directories(self, mock_run, mock_chmod, mock_makedirs, script_path, temp_env_file):
