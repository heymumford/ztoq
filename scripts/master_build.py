...
def run_tests() -> TestSuiteResults:
    ...
    # Run unit tests
    returncode, stdout, stderr = run_command(["poetry", "run", "pytest", "tests/unit", "-v"], ROOT_DIR)
    parse_test_results(stdout, results.unit)
    
    # Run integration tests
    returncode, stdout, stderr = run_command(["poetry", "run", "pytest", "tests/integration", "-v"], ROOT_DIR)
    parse_test_results(stdout, results.integration)
    
    # Run e2e tests if they exist
    if (TESTS_DIR / "e2e").exists():
        returncode, stdout, stderr = run_command(["poetry", "run", "pytest", "tests/e2e", "-v"], ROOT_DIR)
...
def create_test_pyramid_ascii(results: TestSuiteResults, test_counts: Dict[str, int]) -> str:
    ...
    e2e_files = test_counts["e2e"]
    integration_files = test_counts["integration"]
    unit_files = test_counts["unit"]
...
def generate_badges(results: TestSuiteResults) -> None:
    ...
    total = results.total
    if total.total > 0:
        pass_pct = (total.passed / total.total) * 100
    else:
        pass_pct = 0
    ...
    with open(BADGE_PATH / "integration_tests.json", "w") as f:
        int_pct = (results.integration.passed / results.integration.total) * 100 if results.integration.total > 0 else 0
        json.dump({
            "schemaVersion": 1,
...
def build_project() -> int:
    ...
    print(f"Found tests: {test_counts['unit']} unit, {test_counts['integration']} integration, {test_counts['e2e']} e2e")
    ...
    # Build documentation
    returncode, stdout, stderr = run_command(["poetry", "run", "python", "build.py", "docs"], ROOT_DIR)
    if returncode != 0:
